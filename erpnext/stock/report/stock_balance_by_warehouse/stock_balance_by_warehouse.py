...
def execute(filters=None):
	...
	df = df.groupby([1]).sum()
	final_data =  [[i for i in row] for row in df.itertuples()]
	#frappe.throw(_(final_data))
	#frappe.throw(_(final_data))
	#final_data= group_by(data, warehouse, 3)
...
# , item_name, description, item_group, brand, stock_uom Removed from query
	if filters.get('show_variant_attributes', 0) == 1:
